...
#ifndef _EPROSIMA_DDS_MONITOR_CONTROLLER_H
#define _EPROSIMA_DDS_MONITOR_CONTROLLER_H
...
public slots:
    ...
    //! Returns the eProsima DDS Monitor version used to compile the Monitor
    QString dds_monitor_version();

    //! Returns the system information for which Fast DDS is built
    QString system_info();

    //! Returns the date on which DDS Monitor was built
    QString build_date();

    //! Returns the DDS Monitor GitHub commit built
    QString git_commit();
...
#endif // _EPROSIMA_DDS_MONITOR_CONTROLLER_H
